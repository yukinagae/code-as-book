...
export class EntryItem extends React.Component {
    ...
    _onClick() {
        ...
        if(this.props.type === 'file') {
            this.props.onClickFile(this.props.content);
            return;
        }
    ...
    render() {
        ...
        if(this.state.subFolders) {
            subFolders = this.state.subFolders.map((entry, index) => {
                return (
                    <li key={index}><EntryItem name={entry.name} type={entry.type} content={entry.content} onClickFile={this.props.onClickFile} /></li>
                    );
            });
        }
...
export default class EntryList extends React.Component {
    ...
    _onClickFile(content) {
        this.props.onClickFile(content);
    }

    render() {
        var entries = this.props.entries.map((entry, index) => {
            return <EntryItem key={index} name={entry.name} type={entry.type} content={entry.content} onClickFile={this._onClickFile.bind(this)} />
        });
